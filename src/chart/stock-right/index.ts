...
class StockRight extends ChartBase {
  ...
  private packing = (node: SVGGSelection, data: FillData) => {
    ...
    if (_tags?.length) {
      ...
      const tag = tags.append('g').attr('transform', `translate(35,33)`);
      _tags.forEach((item) => {
        const content = typeof item === 'string' ? item : item.text ?? '';
        const _fill =
          typeof item === 'string' ? '#BB833D' : item.fill ?? '#BB833D';
    ...
    if (desc) {
      ...
      if (typeof desc === 'string') {
        descs.append('text').attr('font-size', 12).append('tspan').text(desc);
    ...
    icon
      ...
      .attr(
        'style',
        'stroke: rgb(153, 153, 153); fill: rgb(255, 255, 255); stroke-width: 1;',
      )
      .attr('r', 5);
    icon
      .append('line')
  ...
  private animation = () => {
    ...
    stackFrame<FillData>(stack, (item) => {
      ...
      item.__line
        ?.attr('d', `M${x1},${y1}L${x1},${y2}L${x2},${y2}`)
        .attr('opacity', 1);
    });
  };
  get onrequest() {
